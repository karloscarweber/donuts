...
# camping overwrite and a skeleton for how we want to layout the application server stuff in the future.
...
module Camping
  ...
  module Controllers
    ...
    class << self
      ...
      #   end
      def R *u
        r=@r # @r represents our routes
        Class.new {
          meta_def(:urls){u}
          meta_def(:inherited){|x|r<< x}
          send :include,CamperCore
        }
      end
      ...
      def M(p)
        ...
        constants.map { |c|
          k = const_get(c)
          k.send :include,C,X,Base,Helpers,Models,CamperCore
          @r=[k]+@r if @r-[k]==@r
